      ...
      @logger.error("Invalid RSS feed", :exception => e)
        ...
        def handle_rss_response(queue, item)
          @codec.decode(item.description) do |event|
            ...
            event.set("description", item.description)
            event.set("audiourl", item.enclosure.url) if !item.enclosure.nil?
            event.set("guid", item.guid.content.strip) if !item.guid.nil?
      #      event.set("itauthor", item.itunes:author) if !item.itunes:author.nil?
            decorate(event)
            queue << event
          end
        end
